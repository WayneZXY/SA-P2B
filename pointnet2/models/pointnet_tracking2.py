...
from pointnet2.utils.pointnet2_modules import PointnetSAModule, PointnetFPModule2
    ...
    def forward(self, template, search):
        ...
        if self.test == False:  # 附加任务只在训练时进行
            ...
            #         new_feature[i] = self.aux_modules[i](search_xyz_list[2 - i], search_xyz_list[3 - i], search_feature_list[2 - i], new_feature[i - 1])
                new_feature[i] = self.aux_modules(search_xyz_list[0], search_xyz_list[3 - i], search_feature_list[3 - i])
            new_features = 0.33 * new_feature[0] + 0.33 * new_feature[1] + 0.33 * new_feature[2]
            aux_feature = new_features.transpose(1, 2)
