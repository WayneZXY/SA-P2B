...
class Pointnet_Tracking3(nn.Module):
    ...
    def forward(self, template, search):
        ...
        """
        # template_xyz_list, template_feature_list, template_feature = self.backbone_net(template, [128, 64, 32])
        template_xyz_list, template_feature_list, template_feature = self.backbone_net(template, [256, 128, 64])
        # 分别为B * 64 * 3，B * d1 * 64
        search_xyz_list, search_feature_list, search_feature = self.backbone_net(search, [256, 128, 64])
        # 分别为B * 128 * 3，B * d2 * 128
