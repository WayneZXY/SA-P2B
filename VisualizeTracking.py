...
def main(args):
    ...
    params = {
        'legend.fontsize': 'x-large',  # 图例字体大小
        'figure.figsize': (15, 5),  # 图像大小
        'axes.labelsize': 'x-large',  # 标签大小
        'axes.titlesize': 'x-large',  # 标题大小
        'xtick.labelsize': 'x-large',  # x坐标标签大小
        'ytick.labelsize': 'x-large',  # y坐标标签大小
        'figure.max_open_warning': 1000  # 最大打开的图像数目
    }

    pylab.rcParams.update(params)
    # rcParams是matplotlib类使用的实例，控制自定义图形的各种参数
    ...
    if "PreTrained" in args.model_name:
        AE_chkpt_file = os.path.join("..", "models", "Pretrain_ShapeNet", "model.pth.tar")
        chkpt_file = None
    if "Ours" in args.model_name:
        AE_chkpt_file = os.path.join("..", "models", "Pretrain_ShapeNet", "model.pth.tar")
        chkpt_file = os.path.join("..", "models", "Ours", "model.pth.tar")

    model = Model(128, chkpt_file=chkpt_file, AE_chkpt_file=AE_chkpt_file).cuda()
    model.eval()  # 调到评估模式

    # Create Dataset
    dataset = SiameseTest(model=model, path=args.path_KITTI, split="Test", scale_BB=1.25)
    ...
    for i in tqdm(range(1, len(PCs))):
        ...
        candidate_PCs_torch = torch.cat(candidate_PCs_reg, dim=0).cuda()  # 列表拼成张量
        ...
        model_PC_encoded = regularizePC(model_PC, model).repeat(tuple(repeat_shape)).cuda()
        ...
        if "PreTrained" in args.model_name:
            ...
            model_PC_decoded.points = model_PC_decoded.points * np.atleast_2d(normalizer).T
        ...
        ratio = 1
        ax.scatter(
            view_PC.points[0, ::ratio],  # ::ratio表示从开头到结尾以ratio为步进，依次读出数据
        ...
        surf = ax.plot_surface(
            ...
            rstride=1,  # 行步进，与曲面上的单位曲面大小有关，正整数
            cstride=1,  # 列步进，与曲面上的单位曲面大小有关，正整数
            linewidth=0.3,
        ...
        ax.zaxis.set_major_locator(LinearLocator(10))  # Z轴的刻度以10为步长
        ax.zaxis.set_major_formatter(FormatStrFormatter('%.02f'))  # Z轴标签文本的格式

        # Add a color bar which maps values to colors.
        fig.colorbar(surf, shrink=0.5, aspect=5)  # 颜色表尺，shrink控制整体大小，aspect控制长宽比例

        ax.w_xaxis.set_pane_color((0.9, 0.9, 0.9, 0.2))  # Set pane color to a RGBA tuple
        ...
        os.makedirs(os.path.join(args.path_results, f"{args.track:04.0f}", "Tracking"), exist_ok=True)
        plt.savefig(
            os.path.join(args.path_results, f"{args.track:04.0f}", "Tracking", f"{i}_{args.model_name}.png"),                  
            format='png',
            dpi=100)
