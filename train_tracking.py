...
def aux_loss(pts_labels, center_targets, point_cls, point_reg):  # 附加任务的损失函数
	r'''
		...
		point_reg = estimation_offset, B * 1024 * 3
	'''
	N = len(pts_labels)
...
for epoch in range(opt.nepoch):
	...
	for i, data in enumerate(tqdm(train_dataloader, position=0, desc='trianing datas')):
		...
		label_point_set.requires_grad_(False)  # 对应search，大小为B * 1024 * 3
		label_cla.requires_grad_(False)  # 对应分类标签，大小为B * 128
		label_reg.requires_grad_(False)  # 对应边界框回归信息，大小为B * 128 * [三维坐标，偏移]，即B * 128 * 4
		object_point_set.requires_grad_(False)  # 对应template，大小为B * 512 * 3
		sample_seg_label.requires_grad_(False)  # 对应search的逐点标签，用于附加分割任务，大小为B * 1024
		sample_seg_offset.requires_grad_(False)  # 对应search的逐点偏移量，用于附加回归任务，大小为B * 1024 * 3
		...
		loss = loss_reg + 0.2 * loss_cla + 1.5 * loss_objective + 0.2 * loss_box + 0.9 * loss_aux_seg + 2 * loss_aux_offset
	...
	for i, data in enumerate(tqdm(test_dataloader, position=0, desc='evaluating datas')):
		...
		val_label_point_set.requires_grad_(False)  # 对应search，大小为B * 1024 * 3
		val_label_cla.requires_grad_(False)  # 对应分类标签，大小为B * 128
		val_label_reg.requires_grad_(False)  # 对应边界框回归信息，大小为B * 128 * [三维坐标，偏移]，即B * 128 * 4
		val_object_point_set.requires_grad_(False)  # 对应template，大小为B * 512 * 3
